...
# Lấy danh sách voice tiếng Anh (dùng pyttsx3 để lấy danh sách cho Test Voice)
engine = pyttsx3.init()
...
def convert_to_speech_thread():
    try:
        ...
        # Tạo giọng đọc với gTTS
        tts = gTTS(text, lang=language, slow=(speed < 1.0))
        ...
        # Nếu speed khác 1.0 thì dùng pydub để thay đổi tốc độ
        if speed != 1.0:
            ...
            new_sound.export(output_filename, format="mp3")
...
def main():
    ...
    speed_scale = tk.Scale(root, from_=0.1, to=2.0, orient="horizontal", resolution=0.1)
    speed_scale.set(1.0)
